...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Atualizar nome, tipo, email e senha
    if ($senha) {
        $stmt = $pdo->prepare("UPDATE usuarios SET nome=:nome, tipo=:tipo, email=:email, senha=:senha WHERE Id=:Id");
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['mensagem'] = "Perfil do Admin Editado com sucesso!";
    } else {
        $_SESSION['mensagem'] = "Erro ao Editar o Perfil do Admin.";
    }

    header('Location: ../Visao/EditarPerfilAdmin.php');
    exit();
}
?>
