...
class login extends usuarios {
    ...
    // Configurações de hash (Argon2id ou bcrypt)
    ...
    public function logar($email, $senha) {
        ...
        if (empty($email) || empty($senha)) {
            $_SESSION['mensagem_login'] = 'Preencha todos os campos.';
            $_SESSION['tipo_alerta_login'] = 'warning';
        ...
        if ($stmt->rowCount() === 0) {
            $_SESSION['mensagem_login'] = 'Erro! Usuário ou senha incorretos.';
            $_SESSION['tipo_alerta_login'] = 'danger';
            $this->redirecionarLogin();
        }
        ...
        if ($dados['tipo'] === 'Docente' && (int)$dados['ativo'] === 0) {
            $_SESSION['mensagem_login'] = 'Conta de docente ainda não foi ativada.';
            $_SESSION['tipo_alerta_login'] = 'warning';
            $this->redirecionarLogin();
        }
        ...
        // Mensagem sucesso para login.php
        $_SESSION['mensagem_login'] = 'Login feito com sucesso!';
        $_SESSION['tipo_alerta_login'] = 'success';
    ...
    public function criarconta(
        ...
        if (empty($nome) || empty($email) || empty($senha) || empty($tipo)) {
            $_SESSION['mensagem_registro'] = 'Preencha todos os campos obrigatórios.';
            $_SESSION['tipo_alerta_registro'] = 'warning';
            $this->redirecionarRegistro();
        }

        if ($tipo === "Estudante" && (empty($matricula) || empty($curso_id) || empty($ano_curricular))) {
            $_SESSION['mensagem_registro'] = 'Matrícula, curso e ano curricular são obrigatórios para estudantes.';
            $_SESSION['tipo_alerta_registro'] = 'warning';
            $this->redirecionarRegistro();
        }

        if ($tipo === "Docente" && empty($departamento_id)) {
            $_SESSION['mensagem_registro'] = 'Departamento é obrigatório para docentes.';
            $_SESSION['tipo_alerta_registro'] = 'warning';
            $this->redirecionarRegistro();
        }
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $_SESSION['tipo_alerta_registro'] = 'danger';
            $this->redirecionarRegistro();
        }
        ...
        if ($this->existeEmail($email)) {
            $_SESSION['mensagem_registro'] = 'E-mail já está em uso.';
            $_SESSION['tipo_alerta_registro'] = 'danger';
            $this->redirecionarRegistro();
        }

        if ($this->existeNome($nome)) {
            $_SESSION['mensagem_registro'] = 'Nome de usuário já está em uso.';
            $_SESSION['tipo_alerta_registro'] = 'danger';
            $this->redirecionarRegistro();
        }
        ...
        if ($this->InserirUsuario() > 0) {
            $_SESSION['mensagem_registro'] = 'Conta criada com sucesso.';
            $_SESSION['tipo_alerta_registro'] = 'success';
            unset(
                $_SESSION['old_nome'], $_SESSION['old_email'], $_SESSION['old_tipo'], 
                $_SESSION['old_matricula'], $_SESSION['old_curso_id'], 
                $_SESSION['old_departamento'], $_SESSION['old_ano']
            );
            $this->redirecionarSucesso();  // para sucesso.php
        } else {
            $_SESSION['mensagem_registro'] = 'Erro! Não foi possível criar esta conta.';
            $_SESSION['tipo_alerta_registro'] = 'danger';
            $this->redirecionarRegistro();
        }
    }
    ...
    private function redirecionarPainel($tipo) {
        ...
        switch ($tipo) {
            ...
            default: header("Location: $url/SGPA/Visao/login.php"); break;
        }
        exit();
    }
...
if (isset($_GET['sair'])) {
    ...
    // Mensagem de logout específica
    $_SESSION['mensagem_logout'] = 'Logout feito com sucesso!';
    $_SESSION['tipo_alerta_logout'] = 'success';

    // Remove todas variáveis da sessão exceto as mensagens de logout
    foreach ($_SESSION as $key => $value) {
        if (!in_array($key, ['mensagem_logout', 'tipo_alerta_logout'])) {
            unset($_SESSION[$key]);
        }
    }
