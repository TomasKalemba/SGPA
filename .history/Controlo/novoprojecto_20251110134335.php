...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (preg_match('/^\d+$/', $titulo) || preg_match('/^\d+$/', $descricao)) {
        $_SESSION['mensagem'] = "⚠️ Título e descrição não podem conter apenas números.";
        header("Location: ../visao/novoprojecto.php");
        exit;
    }

    if (mb_strlen($titulo) > 255 || mb_strlen($descricao) < 10) {
        $_SESSION['mensagem'] = "⚠️ Verifique o tamanho do título ou da descrição.";
        header("Location: ../visao/novoprojecto.php");
        exit;
    }
    ...
    if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($extensao, $tiposPermitidos)) {
            $_SESSION['mensagem'] = "⚠️ Tipo de arquivo inválido.";
            header("Location: ../visao/novoprojecto.php");
            exit;
        }

        if ($tamanhoArquivo > 10 * 1024 * 1024) {
            $_SESSION['mensagem'] = "⚠️ Arquivo ultrapassa 10MB.";
            header("Location: ../visao/novoprojecto.php");
            exit;
        }
        ...
        if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $caminhoFinal)) {
            $caminhoArquivo = $nomeFinal;
        } else {
            $_SESSION['mensagem'] = "⚠️ Falha ao salvar arquivo.";
            header("Location: ../visao/novoprojecto.php");
            exit;
        }
    }
    ...
    try {
        ...
        $stmtDocente = $conn->prepare("SELECT fk_usuarios_cursos, fk_departamento FROM usuarios WHERE id = ?");
        $stmtDocente->execute([$docente_id]);
        $docenteInfo = $stmtDocente->fetch(PDO::FETCH_ASSOC);

        $docenteCurso = $docenteInfo['fk_usuarios_cursos'];
        $docenteDepto = $docenteInfo['fk_departamento'];
        ...
        $stmtBuscarEstudante = $conn->prepare("
            ...
            FROM usuarios u
            JOIN cursos c ON u.fk_usuarios_cursos = c.id
            WHERE u.id = ? 
              AND u.tipo = 'Estudante'
              AND u.fk_departamento = ?
              AND c.departamento = ?
        ");

        foreach ($estudantes as $estudante_id) {
            $stmtBuscarEstudante->execute([$estudante_id, $docenteDepto, $docenteDepto]);
