...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (preg_match('/^\d+$/', $titulo) || preg_match('/^\d+$/', $descricao)) {
        $_SESSION['mensagem'] = "⚠️ O título ou a descrição não pode conter apenas números.";
        header("Location: ../visao/novoprojecto.php");
        exit;
    }

    if (mb_strlen($titulo) > 255 || mb_strlen($descricao) < 10) {
        $_SESSION['mensagem'] = "⚠️ Tamanho inválido para título ou descrição.";
        header("Location: ../visao/novoprojecto.php");
        exit;
    }
    ...
    if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($tamanhoArquivo > 10 * 1024 * 1024) {
            $_SESSION['mensagem'] = "⚠️ Arquivo maior que 10MB.";
            header("Location: ../visao/novoprojecto.php");
            exit;
        }
    ...
    try {
        ...
        // Adicionar estudantes do mesmo curso/departamento
        foreach ($estudantes as $estudante_id) {
            ...
            if (!$row) {
                $_SESSION['mensagem'] = "⚠️ Estudante ID $estudante_id não pertence ao seu curso/departamento.";
                $conn->rollBack();
                header("Location: ../visao/novoprojecto.php");
                exit;
            }
