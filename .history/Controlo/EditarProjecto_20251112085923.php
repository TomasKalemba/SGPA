...
// Se for POST, edita projeto e estudantes
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($grupo) {
            ...
            // Remove apenas estudantes que não estão na lista enviada
            $stmtDelete = $pdo->prepare("DELETE FROM grupo_estudante WHERE grupo_id = ? AND estudante_id NOT IN (" . (count($estudantes) ? implode(',', $estudantes) : '0') . ")");
            $stmtDelete->execute([$grupo_id]);

            // Insere novos estudantes que ainda não estão vinculados
