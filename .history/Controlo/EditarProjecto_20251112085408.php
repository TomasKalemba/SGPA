...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $caminho_arquivo)) {
            ...
            if (!empty($arquivoAtual) && file_exists($diretorio_upload . $arquivoAtual)) {
                unlink($diretorio_upload . $arquivoAtual);
            }
        } else {
    ...
    try {
        ...
        $sql = "UPDATE projectos 
                SET titulo=:titulo, descricao=:descricao, docente_id=:docente_id, 
                    data_criacao=:data_criacao, prazo=:prazo, arquivo=:arquivo, feedback=:feedback 
                WHERE Id=:Id";
        ...
        if ($stmt->execute()) {
            ...
            if ($grupo) {
                ...
                // Remove todos os estudantes do grupo
