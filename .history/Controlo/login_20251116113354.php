...
class login extends usuarios {
    ...
    public function logar($email, $senha) {
        ...
        // ** Mensagem de sucesso para aparecer no login.php ao entrar **
...
// Logout com mensagem de sucesso por 2 segundos
if (isset($_GET['sair'])) {
    if (session_status() === PHP_SESSION_NONE) session_start();
    ...
    // Define mensagem para logout
    $_SESSION['mensagem'] = 'Logout feito com sucesso!';
    $_SESSION['tipo_alerta'] = 'success';

    // Remove variáveis da sessão, exceto mensagem e tipo_alerta
    foreach ($_SESSION as $key => $value) {
        if (!in_array($key, ['mensagem', 'tipo_alerta'])) {
            unset($_SESSION[$key]);
        }
    }
