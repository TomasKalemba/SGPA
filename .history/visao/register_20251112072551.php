...
$sql = "SELECT id, nome, departamento FROM cursos ORDER BY nome ASC";
                ...
                <div class="card-body">
                    ...
                    <?php if (isset($_SESSION['mensagem']) && !empty($_SESSION['mensagem'])): ?>
                        <?php
                            ...
                            $tipoAlerta = 'info';
                            if (str_contains(strtolower($mensagem), 'sucesso')) $tipoAlerta = 'success';
                            elseif (str_contains(strtolower($mensagem), 'erro') || str_contains(strtolower($mensagem), 'já está em uso')) $tipoAlerta = 'danger';
                            elseif (str_contains(strtolower($mensagem), 'aguarde')) $tipoAlerta = 'warning';
                        ?>
...
document.getElementById("tipoUsuario").addEventListener("change", function() {
    ...
    if (tipo === "Estudante") {
        ...
        matriculaInput.addEventListener("input", function() { this.value = this.value.replace(/\D/g, ''); });

        const form = document.getElementById("createAccountForm");
        form.addEventListener("submit", function(e) {
            if (matriculaInput.value.length !== 6) {
