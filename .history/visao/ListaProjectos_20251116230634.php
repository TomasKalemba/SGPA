...
// Verifica o tipo de usuário e busca os dados apropriados
if ($_SESSION['tipo'] === 'Admin') {
    include_once('head/Admin.php');
    $submissoes = $submissoesDAO->getSubmissoesParaAdmin();
    $projectos = $projectoDAO->getTodosProjetos(); // Admin tem acesso a todos os projetos
} else {
    include_once('head/headDocente.php');
    // Docente verá apenas projetos por ele criados ou submetidos para ele
    $submissoes = $submissoesDAO->getSubmissoesParaDocente($_SESSION['id']);
    $projectos = $projectoDAO->getProjetosPorDocente($_SESSION['id']); // Projetos criados/submetidos pelo docente
}

// Normaliza os IDs para garantir que sejam usados de forma consistente
                ...
                <div class="tab-content" id="conteudoAbas">
                    ...
                    <div class="tab-pane fade show active" id="lista" role="tabpanel">
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle text-center tabela-projetos" id="tabelaProjetos">
                    ...
                    <div class="tab-pane fade" id="submetidos" role="tabpanel">
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle text-center tabela-projetos" id="tabelaSubmetidos">
                                <thead class="table-dark">
                                    <tr>
                                        <th>N.º</th>
                                        <th>Enviado por:</th>
...
<script>
    $(document).ready(function() {
        ...
        function atualizarTabelaProjetos() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // Inicializa DataTable para filtros, pesquisa e exportação na tabela de projetos
                    $('#tabelaProjetos').DataTable({
                        dom: 'Bfrtip', // Layout de elementos (buttons, filtros, tabela)
                        buttons: [
                            {
                                extend: 'excelHtml5', // Exportação para Excel
                                title: 'Projetos', // Título do arquivo Excel
                                text: 'Exportar para Excel', // Texto do botão
                                className: 'btn btn-success' // Estilo do botão
                            },
                            {
                                extend: 'pdfHtml5', // Exportação para PDF
                                title: 'Projetos', // Título do arquivo PDF
                                orientation: 'landscape', // Orientação da página PDF
                                pageSize: 'A4', // Tamanho da página PDF
                                text: 'Exportar para PDF', // Texto do botão
                                className: 'btn btn-danger' // Estilo do botão
                            },
                            {
                                extend: 'print', // Função de impressão
                                title: 'Projetos', // Título da impressão
                                text: 'Imprimir', // Texto do botão
                        ...
                        pageLength: 10,
                        lengthChange: true,
                        order: [[0, 'asc']], // Ordena a tabela pela primeira coluna (N.º)
                    });
                }
            });
        }

        // Função para inicializar a DataTable para a tabela de "Projetos Submetidos"
        function inicializarTabelaSubmetidos() {
            $('#tabelaSubmetidos').DataTable({
                dom: 'Bfrtip', // Layout de elementos (buttons, filtros, tabela)
                buttons: [
                    {
                        extend: 'excelHtml5', // Exportação para Excel
                        title: 'Projetos Submetidos', // Título do arquivo Excel
                        text: 'Exportar para Excel', // Texto do botão
                        className: 'btn btn-success' // Estilo do botão
                    },
                    {
                        extend: 'pdfHtml5', // Exportação para PDF
                        title: 'Projetos Submetidos', // Título do arquivo PDF
                        orientation: 'landscape', // Orientação da página PDF
                        pageSize: 'A4', // Tamanho da página PDF
                        text: 'Exportar para PDF', // Texto do botão
                        className: 'btn btn-danger' // Estilo do botão
                    ...
                    {
                        extend: 'print', // Função de impressão
                        title: 'Projetos Submetidos', // Título da impressão
                        text: 'Imprimir', // Texto do botão
                        className: 'btn btn-primary' // Estilo do botão
                    }
                ],
        ...
        // Chama as funções para inicializar as tabelas
        atualizarTabelaProjetos();
        inicializarTabelaSubmetidos();
    });
</script>
