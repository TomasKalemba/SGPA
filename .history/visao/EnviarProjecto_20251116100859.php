    ...
    <form method="post" action="../controlo/EnviarProjecto.php" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="docente_nome" 
                value="<?= htmlspecialchars($old['docente_nome'] ?? '') ?>" required>
        </div>

        <!-- Descrição -->
        <div class="mb-3">
            <label class="form-label"><i class="fas fa-pencil-alt"></i> Descrição</label>
            <textarea class="form-control" id="descricaoProjeto" name="descricao" rows="3" readonly required>
<?= htmlspecialchars($old['descricao'] ?? '') ?>
            </textarea>
        </div>
