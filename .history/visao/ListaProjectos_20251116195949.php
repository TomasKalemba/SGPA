...
<main class="container py-4">
    <div class="card shadow">
        ...
        <div class="card-body">
            ...
            <!-- Formulário para adicionar um novo projeto -->
            <form id="adicionarProjetoForm" class="mb-4">
                <div class="row">
                    <div class="col-md-4">
                        <input type="text" id="titulo" name="titulo" class="form-control" placeholder="Título do Projeto" required>
                    </div>
                    <div class="col-md-4">
                        <input type="text" id="descricao" name="descricao" class="form-control" placeholder="Descrição do Projeto" required>
                    </div>
                    <div class="col-md-2">
                        <input type="datetime-local" id="prazo" name="prazo" class="form-control" required>
                    </div>
                    <div class="col-md-2">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if (!empty($p['arquivo'])): ?>
                                                <a href="download.php?file=<?= urlencode($p['arquivo']) ?>" class="btn btn-success btn-sm"><i class="fas fa-download"></i> Download</a>
                                            <?php else: ?>
                                                <span class="text-muted">Nenhum</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="d-flex justify-content-center gap-2">
                                            <a href="../visao/Editar.php?id=<?= htmlspecialchars($p['id']) ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if (!empty($s['arquivo'])): ?>
                                                <a href="../uploads/<?= rawurlencode($s['arquivo']) ?>" download class="btn btn-success btn-sm"><i class="fas fa-download"></i> Download</a>
                                            <?php else: ?>
                                                <span class="text-muted">Nenhum</span>
                                            <?php endif; ?>
                                        </td>
...
<script>
    $(document).ready(function () {
        ...
        $('#adicionarProjetoForm').submit(function(e) {
            ...
            $.ajax({
                url: '../controlo/NovoProjeto.php', // Caminho para o script PHP
                type: 'POST',
                data: formData,
                dataType: 'json',
                success: function(response) {
                    if (response.error) {
                        ...
                        table.row.add([
                            ...
                            response.grupo,
                            response.prazo,
                            '<a href="download.php?file=' + response.arquivo + '" class="btn btn-success btn-sm"><i class="fas fa-download"></i> Download</a>',
                            '<a href="../visao/Editar.php?id=' + response.id + '" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>' +
                            '<a href="../controlo/EliminarProjecto.php?id=' + response.id + '" class="btn btn-danger btn-sm" onclick="return confirm(\'Tem certeza que deseja eliminar este projeto?\');"><i class="fas fa-trash-alt"></i></a>'
                        ]).draw(false);
                    }
                },
                error: function() {
