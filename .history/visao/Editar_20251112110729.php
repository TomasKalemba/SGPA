...
// Cabeçalhos por tipo de usuário
...
<div id="layoutSidenav_content">
    ...
    <!-- Select de estudantes já vinculados, permite remover -->
    <div class="form-group mb-3">
        <label for="estudantes_existentes"><i class="fas fa-users"></i> Estudantes do Projeto</label>
        <select id="estudantes_existentes" name="estudantes[]" class="form-control" multiple="multiple" style="width:100%;">
            <?php foreach($estudantes_vinculados as $est): ?>
                <option value="<?= $est['id'] ?>" selected><?= htmlspecialchars($est['nome']) ?></option>
            <?php endforeach; ?>
        </select>
        <small class="text-muted">Remova estudantes do grupo se necessário.</small>
    </div>

    <!-- Select de novos estudantes via AJAX -->
    <div class="form-group mb-3">
        <label for="novos_estudantes"><i class="fas fa-user-plus"></i> Adicionar Novo(s) Estudante(s)</label>
        <select id="novos_estudantes" name="novos_estudantes[]" class="form-control" multiple="multiple" style="width:100%;">
            <!-- Inicialmente vazio -->
        </select>
        <small class="text-muted">Busque e selecione estudantes para adicionar ao grupo do projeto.</small>
    </div>

    <script>
    $(document).ready(function() {
        // Select de estudantes existentes (apenas remover)
        $('#estudantes_existentes').select2({
            placeholder: "Remova estudantes se necessário"
        });

        // Select para adicionar novos estudantes via AJAX
        $('#novos_estudantes').select2({
            placeholder: "Buscar estudantes para adicionar...",
            ajax: {
