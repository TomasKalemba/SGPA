...
<head>
  ...
  <style>
    ...
    * {
    ...
    .input-group input {
      width: 100%;
      padding: 12px 40px; /* espaço para ícones à esquerda e direita */
      border: none;
      border-radius: 30px;
      outline: none;
...
<script>
  // Esconde overlay no carregamento da página para garantir
  window.addEventListener('load', () => {
    document.getElementById('overlayProcessando').style.display = 'none';

    // Mostrar overlay para qualquer mensagem de sucesso
    const alertBox = document.querySelector('.alert-success');
    if (alertBox) {
      const mensagem = alertBox.textContent.trim();
      const overlay = document.getElementById('overlayProcessando');
      overlay.querySelector('div:nth-child(2)').textContent = mensagem;
      overlay.style.display = 'fl<?php
session_start();
    ...
    <div class="right-box">
      <div style="width: 100%; display: flex; flex-direction: column; align-items: center;">
        ...
        <form id="loginForm" method="post" action="../controlo/login.php">
          ...
          <!-- Campo usuário igual ao de senha -->
          <div class="input-group">
            <i class="fas fa-user"></i>
            <input type="email"
                   name="email"
...
<script>
  ...
  // Mostrar overlay ao submeter o formulário
