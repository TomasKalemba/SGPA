...
<i class="bi bi-bar-chart-line"></i> Gráfico de Status dos Projetos
</div>
...
$(document).ready(function(){
    ...
    // 🔹 Carrega dados dos indicadores e atualiza gráfico
    $.getJSON('dados_resumo.php', function(data){
        const em_andamento = data.em_andamento ?? 0;
        const concluidos   = data.concluidos ?? 0;
        const atrasados    = data.atrasados ?? 0;
        const em_falta     = data.em_falta ?? 0;
        const total        = data.total ?? 0;

        // Atualiza cards
        $('#em_andamento').text(em_andamento);
        $('#concluidos').text(concluidos);
        $('#atrasados').text(atrasados);
        $('#em_falta').text(em_falta);
        $('#total').text(total);
        ...
        window.statusChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Projetos',
                    data: [em_andamento, concluidos, atrasados, em_falta, total],
                    backgroundColor: [
                        '#17a2b8','#28a745','#dc3545','#ffc107','#0d6efd'
                    ]
                }]
            },
