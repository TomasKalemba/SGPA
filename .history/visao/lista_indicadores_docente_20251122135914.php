...
.info-cards { display:flex; gap:10px; flex-wrap:wrap; margin-bottom:1.5rem; }
...
<div class="info-cards">
    <div class="card bg-andamento shadow-sm" data-tipo="em_andamento">
        <i class="bi bi-hourglass-split"></i><h6>Em Andamento</h6><h2 id="em_andamento">0</h2>
    </div>
    <div class="card bg-concluidos shadow-sm" data-tipo="concluidos">
        <i class="bi bi-check-circle"></i><h6>Concluídos</h6><h2 id="concluidos">0</h2>
    </div>
    <div class="card bg-atrasados shadow-sm" data-tipo="atrasados">
        <i class="bi bi-clock-history"></i><h6>Atrasados</h6><h2 id="atrasados">0</h2>
    </div>
    <div class="card bg-emfalta shadow-sm" data-tipo="em_falta">
        <i class="bi bi-exclamation-circle"></i><h6>Em Falta</h6><h2 id="em_falta">0</h2>
    </div>
    <div class="card bg-total shadow-sm" data-tipo="total">
        <i class="bi bi-collection-fill"></i><h6>Total</h6><h2 id="total">0</h2>
    </div>
</div>
...
<thead>
<tr><th>#</th><th>Projeto / Status</th><th>Data</th></tr>
</thead>
    ...
    $(".info-cards .card").on("click",function(){
        ...
        $.getJSON('lista_indicadores_docente.php',{tipo:tipo},function(data){
            if(Array.isArray(data)){
                registros = data.map(item => ({nome:item.titulo||item.nome||'', data:item.prazo||item.data||''}));
                const titulos = {
