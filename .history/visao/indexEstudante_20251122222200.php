...
function renderizarListaEstudante(){
    ...
    const filtrados=registrosEstudante.filter(r => {
        const nome=(r.nome||'').toLowerCase();
        const prazo=(r.prazo||'').toLowerCase();
        return nome.includes(busca) || prazo.includes(busca);
    });
    ...
    if(pagina.length===0){
        ...
        pagina.forEach((item,idx)=>{
            $tbody.append(`<tr><td>${inicio+idx+1}</td><td>${item.nome}</td><td>${item.prazo}</td></tr>`);
        });
    }
...
$(".row .card").on("click",function(){
    ...
    $.getJSON('lista_indicadores_estudante.php',{tipo:tipo})
    .done(function(data){
        if(!Array.isArray(data)){
            console.error('Retorno inválido do PHP:', data);
            data = [];
        }
